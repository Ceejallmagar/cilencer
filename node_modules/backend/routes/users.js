...
// Feature a user (Admin only)
router.post('/:userId/feature', verifyToken, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        const rocketBadge = { id: 'rocket', name: 'Rocket', icon: '🚀' };

        // Award 'rocket' badge
        await db.collection('users').doc(userId).update({
            badges: admin.firestore.FieldValue.arrayUnion(rocketBadge.id)
        });
        ...
        await db.collection('notifications').add({
            userId,
            type: 'badge',
            message: `🚀 Congratulations! You've been featured by an admin and earned the Rocket badge!`,
            read: false,
            createdAt: admin.firestore.FieldValue.serverTimestamp()
        });

        res.json({ message: 'User featured successfully' });
    } catch (error) {
        console.error('Feature user error:', error);
        res.status(500).json({ error: 'Failed to feature user' });
    }
});
